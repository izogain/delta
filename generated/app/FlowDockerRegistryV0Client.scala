 ...
 * apibuilder:0.12.3 https://api.apibuilder.io/flow/docker-registry/0.0.4/play_2_4_client
 */
...
package io.flow.docker.registry.v0 {
  ...
  object Constants {
    ...
    val UserAgent = "apibuilder:0.12.3 https://api.apibuilder.io/flow/docker-registry/0.0.4/play_2_4_client"
    val Version = "0.0.4"
    val VersionMajor = 0
    ...
    object DockerRepositories extends DockerRepositories {
      override def postAutobuild(
        ...
        _executeRequest("POST", s"/v2/repositories/${play.utils.UriEncoding.encodePathSegment(org, "UTF-8")}/${play.utils.UriEncoding.encodePathSegment(repo, "UTF-8")}/autobuild/", body = Some(payload), requestHeaders = requestHeaders).map {
          ...
          case r => throw new io.flow.docker.registry.v0.errors.FailedRequest(r.status, s"Unsupported response code[${r.status}]. Expected: 201, 400, 401, 404")
        }
      }
    }
