 ...
 * apibuilder:0.12.38 https://app.apibuilder.io/flow/delta-config/0.3.90/play_2_x_json
 */
package io.flow.delta.config.v0.models {
   ...
   * @param name
   * @param dockerfile
   * @param initialNumberInstances When first deploying this branch, the number of
   * instances we create
   * @param portContainer The port number on the container that is bound to the
   * user-specified or automatically assigned host port.
   * @param portHost The port number on the container instance to reserve for your
   * container
   * @param instanceType
   * @param version The version of Delta to use for deployments. Defaults to 1.0 if
   * not specified
   * @param dependencies The names of other builds that this one is dependent on. If
   * specified, we will ensure that we never scale this build to a tag that is ahead
   * of the minimum version of the dependent application running in production.
   * @param memory The number of MiB of memory to reserve for the container.
   */
  case class Build(
    name: String,
    dockerfile: String,
  ...
  object InstanceType {
     ...
     * above.
     */
    val all: scala.List[InstanceType] = scala.List(C4Large, M4Large, T2Micro, T2Small, T2Medium, T2Large)
