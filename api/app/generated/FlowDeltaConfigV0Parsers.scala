 ...
 * apibuilder 0.14.3 app.apibuilder.io/flow/delta-config/0.6.12/anorm_2_6_parsers
 */
import anorm._
  ...
  object Build {
    ...
    def parser(
      ...
      SqlParser.str(prefixOpt.getOrElse("") + healthcheckUrl).? map {
        case name ~ dockerfile ~ initialNumberInstances ~ instanceType ~ memory ~ portContainer ~ portHost ~ remoteLogging ~ stages ~ dependencies ~ version ~ healthcheckUrl => {
