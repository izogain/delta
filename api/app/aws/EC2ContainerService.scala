...
case class EC2ContainerService @javax.inject.Inject() (
  ...
  **/
  ...
  def registerTaskDefinition(
    ...
    // taken from .delta file or defaults to InstanceTypeDefaults
    val maxMemory = settings.containerMemory

    // memory to allocate at startup - should be pretty safe
    val minMemory = maxMemory / 3

    // bin/[service]-[api|www] script generated by "sbt stage" uses this when passed to JAVA_OPTS below
    val jvmMemorySetting = s"-Xms${minMemory}m -Xmx${maxMemory}m"
