...
import io.flow.delta.v0.models.BinaryType
import org.specs2.mutable._

class DeltaHelperSpec extends Specification with Factories {

  lazy val org = makeOrganizationSummary()

  "crossBuildVersion for scala" in {
    Seq("2.10", "2.10.1", "2.10.1-RC1", "2.10.0-M3").map { tag =>
      DeltaHelper.crossBuildVersion(BinaryType.Scala, tag) must beEqualTo(
  ...
  "crossBuildVersion for other binaries uses whole version" in {
    Seq("2.10", "2.10.1", "2.10.1-RC1", "2.10.0-M3").map { tag =>
      DeltaHelper.crossBuildVersion(BinaryType.UNDEFINED("other"), tag).value must beEqualTo(tag)
    }
  }
